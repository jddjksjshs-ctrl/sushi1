...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (u === "/api/orders" && method === "POST") {
      ...
      const cleanItems = items.map((i) => ({ name: String(i.name), qty: Number(i.qty) || 1, price: Number(i.price) || 0 }));
      const total = cleanItems.reduce((s, i) => s + i.price * i.qty, 0);
      const order = { id: "SB-" + (++db.seq.order), uid: email, name: String(body.name), phone: String(body.phone), addr: String(body.addr), note: body.note ? String(body.note) : "", payment: body.payment ? String(body.payment) : "\u041a\u0430\u0440\u0442\u043a\u0430 \u043e\u043d\u043b\u0430\u0439\u043d", items: cleanItems, total, status: "new", date: new Date().toISOString() };
